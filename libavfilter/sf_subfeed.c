...
typedef struct SubFeedContext {
    ...
    FFFrameQueueGlobal fifo_global;
...
static int init(AVFilterContext *ctx)
{
    ...
    ff_framequeue_global_init(&s->fifo_global);
    ff_framequeue_init(&s->fifo, &s->fifo_global);
...
static int config_output(AVFilterLink *outlink)
{
    ...
    if (s->mode == FM_FORWARD)
        f_outlink->frame_rate = (AVRational) { 1, 0 };
    else
...
static int request_frame(AVFilterLink *outlink)
{
    ...
    int64_t next_pts;
    int64_t interval = ms_to_avtb((int64_t)(av_q2d(av_inv_q(f_outlink->frame_rate)) * 1000));
    AVFrame *out;
    int status;
    ...
    if (!s->got_first_input && f_inlink->current_pts != AV_NOPTS_VALUE) {

        s->got_first_input = 1;
        next_pts = av_rescale_q(f_inlink->current_pts, inlink->time_base, AV_TIME_BASE_Q);
...
static int filter_frame(AVFilterLink *inlink, AVFrame *frame)
{
    ...
    ret = ff_framequeue_add(&s->fifo, frame);
    if (ret < 0)
        return ret;
...
const FFFilter ff_sf_subfeed = {
    .p.name           = "subfeed",
    .p.description    = NULL_IF_CONFIG_SMALL("Control subtitle frame timing and flow in a filtergraph"),
    .p.priv_class     = &subfeed_class,
    .init           = init,
    .uninit         = uninit,
