...
static int config_video_output(AVFilterLink *outlink)
{
    ...
    if (f_inlink->hw_frames_ctx)
        f_outlink->hw_frames_ctx = av_buffer_ref(f_inlink->hw_frames_ctx);
...
static int config_sub_output(AVFilterLink *outlink)
{
    ...
    f_outlink->frame_rate = (AVRational){1000, s->real_time_latency_msec};
...
static int filter_frame(AVFilterLink *inlink, AVFrame *frame)
{
    ...
    if (sd && (s->had_keyframe || frame->flags & AV_FRAME_FLAG_KEY)) {
        int got_output = 0;
...
const FFFilter ff_sf_splitcc = {
    .p.name           = "splitcc",
    .p.description    = NULL_IF_CONFIG_SMALL("Extract closed-caption (A53) data from video as subtitle stream."),
    .p.priv_class     = &split_cc_class,
    .init           = init,
    .uninit         = uninit,
