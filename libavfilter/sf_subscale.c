...
static av_cold int preinit(AVFilterContext *ctx)
{
    ...
    ret = av_opt_set_int(s->sws, "threads", 0, 0);
    if (ret < 0)
        return ret;
...
static int config_output(AVFilterLink *outlink)
{
    ...
    ff_scale_adjust_dimensions(inlink, &s->w, &s->h,
                               s->force_original_aspect_ratio, 2, 1);
                           ...
                           int dst_linesize, uint8_t *dst_data, uint32_t *dst_pal)
{
    ...
    ret = avpriv_elbg_do(&s->ctx, s->codeword, 4, codeword_length, s->codebook, s->num_output_colors, 1, s->codeword_closest_codebook_idxs, &s->lfg, 0);
...
static int filter_frame(AVFilterLink *inlink, AVFrame *frame)
{
    ...
    if (ret >= 0) {
        ...
        if (!in_w || !in_h || !out_w || !out_h) {
            in_w = in_h = out_w = out_h = 1;
        }

        const AVRational x_factor = { .num = out_w, .den = in_w };
        const AVRational y_factor = { .num = out_h, .den = in_h };
        ...
        av_log(inlink->dst, AV_LOG_DEBUG, "subscale output - size %dx%d  pts: %"PRId64"  areas: %u\n", frame->width, frame->height, frame->subtitle_timing.start_pts, frame->num_subtitle_areas);
...
const FFFilter ff_sf_subscale = {
    .p.name            = "subscale",
    .p.description     = NULL_IF_CONFIG_SMALL("Scale graphical subtitles."),
    .preinit           = preinit,
    .init              = init,
    .p.priv_class      = &subscale_class,
    .uninit            = uninit,
