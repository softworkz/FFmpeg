...
static int config_output(AVFilterLink *outlink)
{
    ...
    f_outlink->frame_rate = ff_filter_link(ctx->inputs[MAIN])->frame_rate;
...
static int do_blend(FFFrameSync *fs)
{
    ...
    if (s->eval_mode == EVAL_MODE_FRAME) {
        ...
        s->var_values[VAR_N] = (double)f_inlink->frame_count_out;
...
const FFFilter ff_vf_overlaygraphicsubs = {
    .p.name          = "overlaygraphicsubs",
    .p.description   = NULL_IF_CONFIG_SMALL("Overlay graphical subtitles on top of the input."),
    .p.priv_class    = &overlaygraphicsubs_class,
...
const FFFilter ff_svf_graphicsub2video = {
    .p.name          = "graphicsub2video",
    .p.description   = NULL_IF_CONFIG_SMALL("Convert graphical subtitles to video"),
    .p.priv_class    = &graphicsub2video_class,
    .priv_size     = sizeof(OverlaySubsContext),
