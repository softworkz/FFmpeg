...
{
    FilterLink *f_outlink = ff_filter_link(outlink);
    AVFilterContext *ctx = outlink->src;

    outlink->w = ctx->inputs[0]->w;
    outlink->h = ctx->inputs[0]->h;
    outlink->time_base = ctx->inputs[0]->time_base;
    f_outlink->frame_rate = ff_filter_link(ctx->inputs[0])->frame_rate;
...
static int filter_video_frame(AVFilterLink *inlink, AVFrame *frame)
{
    ...
    int64_t time_ms1 = (int64_t)((double)ff_filter_link(ctx->inputs[1])->current_pts * av_q2d(ctx->inputs[1]->time_base) * 1000);
...
static int textsub2video_config_output(AVFilterLink *outlink)
{
    ...
    f_outlink->frame_rate = s->frame_rate;
...
static int textsub2video_request_frame(AVFilterLink *outlink)
{
    ...
    const FilterLink *f_inlink = ff_filter_link(inlink);
    int64_t last_pts = f_outlink->current_pts;
    ...
    if (f_inlink->current_pts == AV_NOPTS_VALUE) { // || outlink->current_pts > inlink->current_pts) {
    ...
    if (last_pts == AV_NOPTS_VALUE)
        next_pts = last_pts = f_inlink->current_pts * av_q2d(inlink->time_base) / av_q2d(outlink->time_base);
    else
        next_pts = last_pts + (int64_t)(1.0 / av_q2d(f_outlink->frame_rate) / av_q2d(outlink->time_base));

    if (s->render_latest_only && f_outlink->current_pts != AV_NOPTS_VALUE && next_pts < s->current_input_pts)
...
const FFFilter ff_vf_overlaytextsubs = {
    .p.name          = "overlaytextsubs",
    .p.description   = NULL_IF_CONFIG_SMALL("Overlay textual subtitles on top of the input."),
    .p.priv_class    = &overlaytextsubs_class,
    .init          = init,
    .uninit        = uninit,
...
const FFFilter ff_svf_textsub2video = {
    .p.name          = "textsub2video",
    .p.description   = NULL_IF_CONFIG_SMALL("Convert textual subtitles to video frames"),
    .p.priv_class    = &textsub2video_class,
    .init          = init,
    .uninit        = uninit,
    .priv_size     = sizeof(TextSubsContext),
