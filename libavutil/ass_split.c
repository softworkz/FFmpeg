...
static const ASSSection ass_sections[] = {
    ...
    {.section = "V4+ Styles", .format_header = "Format", .fields_header = "Style", .size = sizeof(ASSStyle), .offset = offsetof(ASS, styles), .offset_count = offsetof(ASS, styles_count), .fields = {
    ...
    {.section = "Events", .format_header = "Format", .fields_header = "Dialogue", .size = sizeof(ASSDialog), .offset = offsetof(ASS, dialogs), .offset_count = offsetof(ASS, dialogs_count), .fields = {
...
av_cold ASSSplitContext *avpriv_ass_split(const char *buf)
{
...
av_cold void avpriv_ass_split_free(ASSSplitContext *ctx)
{
    if (ctx)
    {
        int i;
